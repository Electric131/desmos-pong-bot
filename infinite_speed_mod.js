// A modded version of the pong script to allow infinite speed - Only really good for this bot unless you are a god (also only really for unstoppable mode)
// Original is at https://github.com/johndoesstuff/randomProjects/blob/master/desmos%20pong.js
...
// Get value from speed
var calcSpeed = function(num) {
	if(gameSpeed < 100) {
  	return num / (100 - gameSpeed)
  }else {
  	return num * (1+((gameSpeed + 1)-100)/100)
  }
};
...
var circularInterval = setInterval(function(){
	if (inGame && gameIn == 1) {
		...
		circularPosition.x += calcSpeed(Math.cos(circularDirection));
		circularPosition.y += calcSpeed(Math.sin(circularDirection));
...
var oneInterval = setInterval(function(){
	if (inGame && gameIn == 2) {	
		...
		ball.x += calcSpeed(3*ball.xvel);
		ball.y += calcSpeed(3*ball.yvel);
		if (ball.x > 4) {
